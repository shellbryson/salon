    ...
    (function() {
        ...
        lunr.Pipeline.load = function(serialised) {
            ...
            serialised.forEach(function(fnName) {
                ...
                if (fn) {
                    ...
                    throw new Error(
                        "Cannot load unregistered function: " + fnName
                    );
                }
            });
        ...
        lunr.Vector.prototype.similarity = function(otherVector) {
            return (
                ...
                (this.magnitude() * otherVector.magnitude())
            );
        };
        ...
        lunr.TokenSet.Builder.prototype.insert = function(word) {
            ...
            if (this.uncheckedNodes.length == 0) {
                ...
                node = this.uncheckedNodes[this.uncheckedNodes.length - 1]
                    .child;
            }
        ...
        lunr.TokenSet.Builder.prototype.minimize = function(downTo) {
            for (var i = this.uncheckedNodes.length - 1; i >= downTo; i--) {
                ...
                if (childKey in this.minimizedNodes) {
                    node.parent.edges[node.char] = this.minimizedNodes[
                        childKey
                    ];
                } else {
                    ...
                    for (var j = 0; j < expandedTerms.length; j++) {
                        ...
                        for (var k = 0; k < clause.fields.length; k++) {
                            ...
                            var field = clause.fields[k],
                                ...
                                matchingDocumentRefs = Object.keys(
                                    fieldPosting
                                ),
                            ...
                            for (
                                 ...
                                 * results
                                 */
                                var matchingDocumentRef =
                                        matchingDocumentRefs[l],
        ...
        lunr.Builder.prototype.add = function(doc) {
            ...
            for (var i = 0; i < this._fields.length; i++) {
                ...
                for (var j = 0; j < terms.length; j++) {
                    ...
                    if (
                        this.invertedIndex[term][fieldName][docRef] == undefined
                    ) {
                        this.invertedIndex[term][fieldName][
                            docRef
                        ] = Object.create(null);
                    }
        ...
        lunr.Builder.prototype.calculateAverageFieldLengths = function() {
            ...
            for (var i = 0; i < this._fields.length; i++) {
                ...
                accumulator[field] =
                    accumulator[field] / documentsWithField[field];
            }
        ...
        lunr.MatchData.prototype.combine = function(otherMatchData) {
            ...
            for (var i = 0; i < terms.length; i++) {
                ...
                for (var j = 0; j < fields.length; j++) {
                    var field = fields[j],
                        keys = Object.keys(
                            otherMatchData.metadata[term][field]
                        );
                    ...
                    for (var k = 0; k < keys.length; k++) {
                        ...
                        if (this.metadata[term][field][key] == undefined) {
                            ...
                            this.metadata[term][field][key] = this.metadata[
                                term
                            ][field][key].concat(
                                otherMatchData.metadata[term][field][key]
                            );
            ...
            if (str_m == str_n) return (this.distance = 0);
                ...
                forEach(str_n, function(n_val, n_idx) {
                    ...
                    forEach(str_m, function(m_val, m_idx) {
                        m_idx++;
                        if (
                            getElem(str_m, m_idx - 1) ==
                            getElem(str_n, n_idx - 1)
                        )
        ...
        $.get(url).done(function(data) {
            ...
            me.index = lunr(function() {
                ...
                idxSrc.forEach(function(arrayItem) {
                    ...
                    if (me.item_preprocessor) {
                        Object.assign(
                            me.blogData[arrayItem.id],
                            me.item_preprocessor(arrayItem)
                        );
                    }
            ...
            try {
                ...
                localStorage.setItem(
                    "ghost_" + subpathKey + "_lunrIndex",
                    JSON.stringify(me.index)
                );
                localStorage.setItem(
                    "ghost_" + subpathKey + "_blogData",
                    JSON.stringify(me.blogData)
                );
                localStorage.setItem(
                    "ghost_" + subpathKey + "_latestPost",
                    me.latestPost
                );
            } catch (e) {
        ...
        loadAPI: function() {
            ...
            if (!this.isInit) {
                ...
                try {
                    ...
                    this.index = localStorage.getItem(
                        "ghost_" + subpathKey + "_lunrIndex"
                    );
                    this.blogData = localStorage.getItem(
                        "ghost_" + subpathKey + "_blogData"
                    );
                    this.latestPost = localStorage.getItem(
                        "ghost_" + subpathKey + "_latestPost"
                    );
                    ...
                    console.warn(
                        "ghostHunter: retrieve from localStorage failed: " + e
                    );
                }
            }
            if (this.isInit) {
                ...
                function() {
                    ...
                    if (thingsFound.length > 1) {
                        ...
                        for (var i = searchResult.length - 1; i > -1; i--) {
                            ...
                            for (
                                j = 0, jlen = thingsFound.length;
                                j < jlen;
                                j++
                            ) {
                                var otherRefs = {};
                                for (
                                    var k = 0, klen = thingsFound[j].length;
                                    k < klen;
                                    k++
                                ) {
                    ...
                    if (searchResult.length === 0) {
                        ...
                        if (this.displaySearchInfo && this.zeroResultsInfo) {
                            results.append(
                                this.format(this.info_template, { amount: 0 })
                            );
                        }
                    ...
                    for (var i = 0; i < searchResult.length; i++) {
                        ...
                        if (postData) {
                            ...
                            console.warn(
                                "ghostHunter: index/data mismatch. Ouch."
                            );
                        }
                    }
                    ...
                    if (currentRefs.length === 0) {
                        for (
                            var i = 0, ilen = resultsData.length;
                            i < ilen;
                            i++
                        ) {
                            results.append(
                        ...
                        var newRefs = [];
                        for (
                            var i = 0, ilen = searchResult.length;
                            i < ilen;
                            i++
                        ) {
